...
describe('Task functions: add and delete', () => {
...
describe('Task functions: edit and clear-complete', () => {
  ...
  it('should update the status of specific indexes', () => {
    ...
    indexes.forEach((index) => {
      const kthLiElement = listDivMock.children[index - 1];
      const bool = kthLiElement.querySelector('input').checked;
  ...
  it('should clear all completed tasks', () => {
    ...
    let storedData = JSON.parse(localStorage.getItem('todoTasks'));
    expect(storedData).toEqual([
      { description: 'Read the documentation', completed: false, index: 1 },
      { description: 'Updated description', completed: false, index: 2 },
      { description: 'Test Task', completed: false, index: 3 },
    ]);
    ...
    storedData = JSON.parse(localStorage.getItem('todoTasks'));
    expect(storedData).toEqual([
